"""Generation of deterministic visual references from frozen benchmark results."""
...
from synthetic_hydropower.model import PlantConfig

PERIOD_LABEL = "Período"
UPPER_RIGHT_LEGEND: Final[Literal["upper right"]] = "upper right"


def render_reference_figures(
    ...
    Args:
        reference_results_dir: Directory containing ``benchmark_time_series.csv``.
        config_path: Frozen fictional plant configuration for level limits.
        figures_dir: Explicit destination for the generated PNG figures.

    Returns:
        Paths to the three generated figures for every scenario.

    Raises:
        FileNotFoundError: If the reference time series or configuration is missing.
    """
    ...
    for scenario_key, scenario_records in records.groupby("scenario", sort=False):
        ...
        figures.extend(
            (
                _render_power(
                    scenario_records, figures_dir, scenario_name, scenario_slug
                ),
                _render_flows(
                    scenario_records, plants, figures_dir, scenario_name, scenario_slug
                ),
                _render_levels(
                    scenario_records, plants, figures_dir, scenario_name, scenario_slug
                ),
...
def _render_power(
    ...
    """Render the grouped power bars for one scenario."""
...
def _render_levels(
    ...
    """Render upstream levels and normal operating limits for both plants."""
